...
class alipayfull_config{
    ...
    function get_configuration (){
        ...
        if (empty($type)){
            $extra_config = [
                ...
                $extra_config = [
                    ...
                    'Type' => 'dropdown',
                    'Options' => [
                        '1' => "</option></select><div class='alert alert-info' role='alert' id='alipay_full_notice' style='margin-bottom: 0px;'>以上信息均可以在 <a href='https://openhome.alipay.com/platform/keyManage.htm?keyType=partner' target='_blank'><span class='glyphicon glyphicon-new-window'></span> 商家支付宝 开放平台</a> 找到 。 如果您需要使用 即时到账 (手机则需要手机网站支付) 功能，您需要填写 卖家支付宝帐户 + 合作伙伴ID + 安全检验码 (MD5)。开启手机网站支付功能前需要开启对应的功能, 否则未申请手机端将不会显示支付界面(显示未签约或其他错误页面)。	</div><div class='alert alert-info' role='alert' id='alipay_full_moblie' style='margin-bottom: 0px;'>如果您需要使用 当面付 功能，您需要填写 应用ID (APPID) + 支付宝公钥 + RSA2(SHA256) 私钥，上三项可以忽略。 </div><script>$('#alipay_full_notice').prev().hide();</script><select style='display:none'>",
                        ],
                    ]
                ];
        }
        $base_config = [
            "FriendlyName" => ['Type' => 'System','Value' => '支付宝扫码支付',],
        ...
        $config["author"] = [
            ...
            'Type' => 'dropdown',
            'Options' => [
                '1' => "</option></select><div class='alert alert-success' role='alert' id='alipay_full_author' style='margin-bottom: 0px;'>Modified by  <a href='https://nyawork.com/' target='_blank'><span class='glyphicon glyphicon-new-window'></span>Nyawork</a> from <a href='https://www.idc.moe/' target='_blank'><span class='glyphicon glyphicon-new-window'></span>ITXE Studio</a>.<br/><span class='glyphicon glyphicon-ok'></span> 支持 WHMCS 5.x-8.x , 当前WHMCS 版本 ".$CONFIG["Version"]."<br/><span class='glyphicon glyphicon-ok'></span> 仅支持 PHP 5.4 以上的环境 , 当前PHP版本 ".phpversion()."</div><script>$('#alipay_full_author').prev().hide();</script><style>* {font-family: Microsoft YaHei Light , Microsoft YaHei}</style><select style='display:none'>",
            ],
        ];
        return $config;
    }
